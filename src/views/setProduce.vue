      ...
      <div>
        <div class="topDiv">
          ...
          <el-row :gutter="20" class="itemRow">
            <el-col v-for="item in produceData.datas" v-bind:key="item.ID" :span="8">
              <el-button
                v-if="deleteState"
                icon="el-icon-close"
                size="mini"
...
export default {
  ...
  data: function() {
    return {
      deleteState: false,
      produceData: new this.$dataModel(
  ...
  methods: {
    addData: function() {
      ...
      newImg.pictureUrl = "";
      this.produceData.newData(newImg, 0);
    },
    saveData: function() {
      let me = this;
      if (me.produceData.getChangedData() == "") {
        return;
      }
      var params={};
      params.loginID=localStorage.getItem("loginID");
      params.loginTime=localStorage.getItem("loginTime");
      params.producesData=me.produceData.getChangedData()
      this.$ajax
        .post("/service/saveProduces", params)
        .then(function(res) {
          if (res.data.result == "success") {
            localStorage.setItem("loginTime", res.data.loginTime);
            me.produceData.acceptChanges();
            me.$message({
              showClose: true,
              message: "保存成功",
              type: "success"
    ...
    deleteData: function(ID) {
      this.produceData.deleteByID(ID);
    },
    ...
    handleAvatarSuccess: function(res) {
      this.produceData.setValueByID("pictureUrl", res, this.opreateID);
    },
    openEditor: function(ID) {
      this.editContent = this.produceData.getValueByID("detail", ID);
      this.editID = ID;
      this.drawer = true;
    },
    editorSure: function(res) {
      this.produceData.setValueByID("detail", res, this.editID);
      this.drawer = false;
    },
    ...
    this.$ajax
      ...
      .then(function(res) {
        if (res.data && res.data.length > 0) {
          me.produceData.loadData(res.data);
        }
      })
      .catch(function(error) {
        me.$message({
